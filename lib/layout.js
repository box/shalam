var path = require('path');

var image = require('./image');


var MIN_WIDTH = 250; // px


exports.createSpriteLayout = function(processors, imageSource) {
    var images = [];
    processors.forEach(function(proc) {
        proc.foundSprites.forEach(function(img) {
            var value = img.value;
            if (images.indexOf(value) === -1) {
                images.push(value);
            }
        });
    });

    images = images.map(function(img) {
        return path.resolve(imageSource, img);
    });

    var renderedImages = images.map(image.fetch);
    renderedImages.sort(function(a, b) {
        return a.width - b.width;
    });

    return performLayout(renderedImages);
};

function performLayout(images) {
    var currentX = 0;
    var currentY = 0;
    var nextY = 0;

    // The the max width of the sprite (MIN_WIDTH or the width of the widest image)
    var max_width = Math.max(
        MIN_WIDTH,
        Math.max.apply(Math, images.map(function(img) {return img.width;}))
    );

    var layout = [];
    for (var i = 0; i < images.length; i++) {
        var img = images[i];

        if (currentX + img.width > max_width) {
            currentX = 0;
            currentY = nextY;
        }

        img.x = currentX;
        img.y = currentY;

        nextY = Math.max(nextY, currentY + img.height);
        currentX += img.width;

        layout.push(img);
    }

    return {
        width: max_width,
        height: nextY,
        images: layout,
    }
}
